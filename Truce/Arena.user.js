...
// @downloadURL https://github.com/robcsaszar/userscripts/raw/master/Truce/Arena.user.js
// @updateURL   https://github.com/robcsaszar/userscripts/raw/master/Truce/Arena.meta.js
...
const setStyles = () => {
  addGlobalStyle(
    `${ARENA_TABLE} > tbody td.YSStyle.highValue { font-weight: 500; color: #45cb85;}`
  );
  addGlobalStyle(
    `${ARENA_TABLE} > tbody td.YSStyle.normalValue { color: #6c6e70;}`
  );
};
...
function getLoot() {
  document
    .querySelector("#tab_ranking")
    .removeEventListener("click", handlePageNavClick);
  updateLoot();
  return document
    .querySelector("#tab_ranking")
    .addEventListener("click", handlePageNavClick);
}

function handlePageNavClick(event) {
  if (event.target.closest(".paginate_button").classList.contains("active"))
    return;
...
async function getOwnStats() {
  ...
  headers.append(
    "Cookie",
    `g_uid=${g_uid}; g_hash=${g_hash}; g_token=${g_token}`
  );
...
function addColumnsToTable(
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    const defenderRank = parseInt(
      currentRow.getElementsByTagName("td")[1]?.innerText
    );
    ...
    if (getRank() == defenderRank) {
      ...
      row.classList.add(
        "YSStyle",
        `${average && array[i] > median ? "highValue" : "normalValue"}`
      );
      row.innerHTML += pre || "";
...
function getLootableGold(row) {
  ...
  return Math.floor(
    (MIN_GOLD +
      Math.pow(attackerLevel, LEVEL_INCREMENT) +
      ADDITIONAL_GOLD * defenderGold) *
      Math.pow(GAP_MODIFIER, rankDifference / MAX_GAP)
  );
}
...
function getExpGain(row) {
  ...
  return Math.floor(
    MIN_EXP +
      Math.pow(attackerLevel, LEVEL_INCREMENT) *
        Math.pow(GAP_MODIFIER, rankDifference / 20)
  );
}

function addGlobalStyle(css) {
  const style =
    document.getElementById("YSStyle") ||
    (function () {
